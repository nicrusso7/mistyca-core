...
public class AWSLambdaConnector {
	...
	public String[] invoke(String json_request, String skill_name) throws InvalidPropertiesFormatException, FileNotFoundException, IOException {
		...
		try {
			...
			//invoke the function
			InvokeResult result_response = client.invoke(req);
			String response_message = "error";
			//check response code
			if(result_response.getStatusCode() == 200) { 
				response_message = new String(result_response.getPayload().array());
			}
			result[0] = String.valueOf(result_response.getStatusCode());
			result[1] = response_message;
		}
		catch(Exception ex) {
			result[0] = "501";
